...
class Game:
	...
	def printfield(self, f):
	    ...
	    for v in range(1,len(l)):
	        text += "\n" + spacing.format(v, f['A'][v],f['B'][v],f['C'][v],f['D'][v] ,f['E'][v] ,f['F'][v] ,f['G'][v] ,f['H'][v] ,f['I'][v],f['J'][v],f['K'][v],f['L'][v],f['M'][v],f['N'][v])#Adds all the rows with the row number to the left and a spacing of 2
	...
	def placeShips(self, player):
	    ...
	    for x in player.ships:
	        ...
	        while self.columnExist(column) == False or row not in rowlist or cellBusy == True:
	            userInput = input(player.name + ", em que celula (A-N)(1-14) você quer colocar sua " + nth[counter] + " barca?\n")
	            if (len(userInput) >= 2):
	        ...
	        while (direction != "right" and direction != "left" and direction != "up" and direction != "down") or self.rowExist(newrow) == False or self.columnExist(newcolumn) == False or cellBusy == True: #loop until the user enters a valid direction
	            direction = input(player.name + ", qual direção (right, left, up or down) seu barco " + nth[counter] + " está virado?\n")
	            cellBusy = False    
	            partCounter = 0
